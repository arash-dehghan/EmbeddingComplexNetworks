...
parser = argparse.ArgumentParser(description=f'Run Experiment{exp} for given starting and ending seed.')
...
for alg in algs:
    for dim in dims:
        ...
        with open(f"results/{filename}","w") as f:
            f.write('experiment,algorithm,delta_exp,best_alpha,best_div_score,best_div_external,best_div_internal,dim,l,f,m,graph_seed,iter,alg_duration,cge_duration\n')
            for graph in graphs:
                ...
                f.write(f'{exp},{alg},{graph[graph.find("param")+5:-4].replace("_",".")},{data[0]},{data[1]},{data[2]},{data[3]},{dim},200,0,diameter,{graph},{ip},{alg_endTime - alg_startTime},{cge_endTime - cge_startTime}\n')
